...
>
  ...
  >
  <form
    class="flex w-full flex-col gap-y-4 text-center"
    action="?/search"
    method="POST"
    use:enhance
  >
          ...
          {#each sortSearchResults as sortOption}
            {#if sortOption.id !== defaultSort.id}
              <DropdownMenu.RadioItem
                value={sortOption.id}
                on:click={() => {
                  results = sortOption.onSelect(results, sortOption.ascending);
  ...
  {#if results && results.length > 0}
    <SecondaryTitle class="self-start">
...
<Container class="md:m-0 m-0 w-full px-0 " minWidth tag="main">
  ...
  {:else if isInit && results.length === 0 && !$page.url.searchParams.get("q")}
    ...
    <SearchSuggestion containerStyling={containterStyling} {setSuggestion} />
  ...
  {:else}
    <div
      class="flex flex-col items-center gap-y-4 md:gap-y-6 {containterStyling}"
    >
