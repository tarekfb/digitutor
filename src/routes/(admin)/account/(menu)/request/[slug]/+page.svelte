<script lang="ts">
  ...
  import { sendRequestMessageSchema } from "src/lib/shared/models/conversation-request.js";
  import ChatWindow from "$lib/components/molecules/chat-window.svelte";
  import { initChat, sendMessageToStore } from "src/stores/chat-request";
  ...
  $: ({ profile: self, request, supabase, session } = data);

  const sendRequestMessageForm = superForm(data.form, {
    validators: zodClient(sendRequestMessageSchema),
    onError: ({ result }) => {
      toast.error(result.error.message);
    },
    invalidateAll: false,
  });
  const { form, allErrors } = sendRequestMessageForm;

  $: chatStore = initChat(request.id, supabase, session);

  $: recipient = self.role == "teacher" ? request.student : request.teacher;
  ...
  const sendMessage = async () => {
    ...
    sendMessageToStore(chatStore, message, request.id, session);
    sendRequestMessageForm.reset();
  };
</script>
